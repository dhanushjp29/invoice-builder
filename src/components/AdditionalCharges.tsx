...
export default function AdditionalCharges({ charges, currency, onChange }: Props) {
  ...
  const addCharge = () => {
    ...
    if (isPresetType(trimmed)) {
      if (charges.some((c) => c.type === trimmed)) {
      ...
      if (charges.some((c) => c.label.toLowerCase() === titled.toLowerCase())) {
        toast.error(`"${titled}" is already added.`);
        return;
      }
        ...
        {charges.length > 0 && (
          <div className="space-y-2">
            {charges.map((charge) => {
              ...
              return (
                <div key={charge._id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
